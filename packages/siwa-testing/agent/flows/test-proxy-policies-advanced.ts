...
async function proxyRequest(
  ...
  const bodyStr = method === 'GET' || method === 'DELETE' ? '' : JSON.stringify(body);
...
export async function testAdvancedPoliciesFlow(): Promise<boolean> {
  ...
  if (adminSecretConfigured) {
    ...
    try {
      const policy = {
        name: 'Test Policy',
        rules: [{ name: 'Test', method: 'sign_message', action: 'ALLOW', conditions: [] }],
      };
      const { status, data } = await proxyRequest(proxyUrl, secret, 'POST', '/policies', policy);
      if (status === 403 && data.error?.includes('Admin authentication required')) {
        pass('Regular secret rejected for policy creation (admin required)');
      } else {
        fail('Regular secret should be rejected', `Status: ${status}, Response: ${JSON.stringify(data)}`);
      }
    } catch (err: any) {
    ...
    if (adminSecret) {
      try {
        const policy = {
          ...
          rules: [{ name: 'Allow all messages', method: 'sign_message', action: 'ALLOW', conditions: [] }],
        };
        const { status, data } = await proxyRequest(proxyUrl, adminSecret, 'POST', '/policies', policy);
        if (status === 201 && data.id) {
          pass(`Admin secret accepted for policy creation \u{2192} ${data.id}`);
          // Clean up
          await proxyRequest(proxyUrl, adminSecret, 'DELETE', `/policies/${data.id}`, {});
        } else {
      ...
      skip('Admin secret policy creation', 'KEYRING_POLICY_ADMIN_SECRET not provided');
    }
    ...
    skip('Admin secret tests', 'KEYRING_POLICY_ADMIN_SECRET not configured on server');
  }
  ...
  const policySecret = adminSecretConfigured && adminSecret ? adminSecret : secret;
  ...
  try {
    ...
    if (status === 201 && data.id) {
      ...
      await proxyRequest(proxyUrl, policySecret, 'POST', `/wallets/${walletAddress}/policies/${data.id}`, {});
  ...
  if (siwaOnlyPolicyId) {
    ...
    try {
      const message = 'example.com wants you to sign in with your Agent account:\n0x1234...';
      const { status, data } = await proxyRequest(proxyUrl, secret, 'POST', '/sign-message', { message });
      if (status === 200 && data.signature) {
        pass('SIWA message allowed by content policy');
      } else {
        fail('SIWA message should be allowed', `Status: ${status}, Response: ${JSON.stringify(data)}`);
      }
    } catch (err: any) {
    ...
    // Clean up
    await proxyRequest(proxyUrl, policySecret, 'DELETE', `/wallets/${walletAddress}/policies/${siwaOnlyPolicyId}`, {});
  }
  ...
  if (lengthPolicyId) {
    ...
    try {
      ...
      const { status } = await proxyRequest(proxyUrl, secret, 'POST', '/sign-message', { message });
      if (status === 200) {
    ...
    // Clean up
    await proxyRequest(proxyUrl, policySecret, 'DELETE', `/wallets/${walletAddress}/policies/${lengthPolicyId}`, {});
  }
  ...
  if (hexPolicyId) {
    ...
    // Clean up
    await proxyRequest(proxyUrl, policySecret, 'DELETE', `/wallets/${walletAddress}/policies/${hexPolicyId}`, {});
  }
  ...
  if (calldataPolicyId) {
    ...
    try {
      const transferData = encodeFunctionData({
        ...
        args: ['0x1234567890123456789012345678901234567890', BigInt('1000000000000000000')],
      });
      const tx = {
    ...
    // Clean up
    await proxyRequest(proxyUrl, policySecret, 'DELETE', `/wallets/${walletAddress}/policies/${calldataPolicyId}`, {});
  }
  ...
  if (recipientPolicyId) {
    ...
    try {
      ...
      if (status === 403) {
        ...
        fail('Transfer to other recipient should be denied', `Status: ${status}`);
      }
    } catch (err: any) {
    ...
    // Clean up
    await proxyRequest(proxyUrl, policySecret, 'DELETE', `/wallets/${walletAddress}/policies/${recipientPolicyId}`, {});
  }
          ...
          conditions: [
            {
              ...
              field: 'contract',
              operator: 'in',
              value: [allowedDelegate, '0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'],
            },
          ],
        },
      ],
  ...
  if (authPolicyId) {
    ...
    try {
      ...
      const { status, data } = await proxyRequest(proxyUrl, secret, 'POST', '/sign-authorization', { auth });
      if (status === 200 && data.address) {
        pass('Authorization to allowed delegate works');
      } else {
        fail('Allowed delegate authorization', `Status: ${status}, Response: ${JSON.stringify(data)}`);
      }
    } catch (err: any) {
    ...
    try {
      ...
      const { status } = await proxyRequest(proxyUrl, secret, 'POST', '/sign-authorization', { auth });
      if (status === 403) {
    ...
    // Clean up
    await proxyRequest(proxyUrl, policySecret, 'DELETE', `/wallets/${walletAddress}/policies/${authPolicyId}`, {});
  }
  ...
  if (authChainPolicyId) {
    ...
    try {
      ...
      if (status === 200 && data.address) {
        ...
        fail('Allowed chain authorization', `Status: ${status}, Response: ${JSON.stringify(data)}`);
      }
    } catch (err: any) {
    ...
    // Clean up
    await proxyRequest(proxyUrl, policySecret, 'DELETE', `/wallets/${walletAddress}/policies/${authChainPolicyId}`, {});
  }
