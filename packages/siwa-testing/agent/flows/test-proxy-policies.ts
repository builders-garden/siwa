...
export async function testProxyPoliciesFlow(): Promise<boolean> {
  ...
  const policySecret: string =
    process.env.KEYRING_POLICY_ADMIN_SECRET || secret;
  ...
  try {
    ...
    if (data.policies_enabled === true) {
      ...
      fail(
        "Health check",
        `Expected policies_enabled: true, got ${JSON.stringify(data)}`
      );
    }
  } catch (err: any) {
  ...
  try {
    ...
    if (status === 200 && data.address && data.policy_id) {
      ...
      pass(
        `Create wallet with default policy \u{2192} ${walletAddress?.slice(
          0,
          10
  ...
  try {
    const { status, data } = await proxyRequest(
      ...
      secret,
      "GET",
      `/wallets/${walletAddress}/policies`,
      {}
    );
    if (
      status === 200 &&
  ...
  try {
    ...
    if (status === 403 && data.error === "Policy violation") {
      ...
      fail(
        "Sign large transaction should be denied",
  ...
  if (denyMessagePolicyId) {
    try {
      ...
      if (status === 403 && data.error === "Policy violation") {
        ...
        fail(
          "Message signing should be blocked",
          `Status: ${status}, Response: ${JSON.stringify(data)}`
        );
      }
    } catch (err: any) {
  ...
  if (customPolicyId) {
    try {
      ...
      if (
        status === 200 &&
        data.policy &&
        data.policy.name === "Updated Spending Limit"
      ) {
  ...
  try {
    ...
    if (status === 201 && data.id) {
      ...
      await proxyRequest(
        ...
        policySecret,
        "POST",
        `/wallets/${walletAddress}/policies/${chainPolicyId}`,
        {}
      );
      ...
      const txRes = await proxyRequest(
        proxyUrl,
        secret,
        "POST",
        "/sign-transaction",
