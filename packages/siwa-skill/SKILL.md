...
1. If **any DENY rule** matches → request is rejected immediately
2. If **any ALLOW rule** matches (and no DENY fired) → request is approved
3. If **no rules match** → request is denied (secure by default)

This means DENY rules always take precedence, and you must have at least one ALLOW rule for any operation to succeed.

### Default Policy

When you create a wallet via `/create-wallet`, a **default policy** is automatically attached. This policy allows:

- Transactions up to **0.1 ETH** in value
- All message signing (for SIWA authentication)
- All EIP-7702 authorization signing

To skip the default policy and use your own, pass `{ "skip_default_policy": true }` when creating the wallet.

### Policy Structure

A policy is a JSON object containing one or more rules. Each rule specifies:

- **method**: Which signing method it applies to (`sign_transaction`, `sign_message`, `sign_authorization`, or `*` for all)
- **action**: `ALLOW` or `DENY`
- **conditions**: An array of conditions that must ALL be true for the rule to fire
...
**Chain restriction** — Only allow transactions on Base:
...
**Contract allowlist** — Only allow interactions with specific contracts:
...
**SIWA messages only** — Only allow signing SIWA authentication messages:
