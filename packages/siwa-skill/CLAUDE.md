...
```bash
cd /home/node/.openclaw/workspace/siwa/packages/siwa-testing && KEYSTORE_BACKEND=proxy pnpm run agent create-wallet
```

### 2. Check Status
Shows your current wallet address, registration state, and keystore backend.
```bash
cd /home/node/.openclaw/workspace/siwa/packages/siwa-testing && KEYSTORE_BACKEND=proxy pnpm run agent status
```

### 3. Register as an Agent
Registers your wallet as an ERC-8004 agent identity (mock mode for testing, or live onchain).
```bash
cd /home/node/.openclaw/workspace/siwa/packages/siwa-testing && KEYSTORE_BACKEND=proxy pnpm run agent register
```
...
```bash
cd /home/node/.openclaw/workspace/siwa/packages/siwa-testing && KEYSTORE_BACKEND=proxy pnpm run agent sign-in
```

### 5. Full Flow (All Steps)
Runs wallet creation → registration → SIWA sign-in → authenticated API call, all sequentially.
```bash
cd /home/node/.openclaw/workspace/siwa/packages/siwa-testing && KEYSTORE_BACKEND=proxy pnpm run agent:flow
```

### 6. Run Proxy Tests
Validates that the keyring proxy is working correctly (7 tests).
```bash
cd /home/node/.openclaw/workspace/siwa/packages/siwa-testing && KEYSTORE_BACKEND=proxy pnpm run agent test-proxy
```
